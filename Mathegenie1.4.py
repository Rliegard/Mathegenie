...
# VERSION: 1.4 (Lehrplan-Sperre mit Toast-Nachricht implementiert)
# ==============================================================================
#
# Eine Tkinter-Anwendung zur Durchführung und Aufzeichnung von Mathematikübungen,
# die in dieser Version um eine adaptive Themenfreigabe erweitert wurde.
#
# Haupt-Features der Version 2.8:
# 1. **Curriculum-Lock (Lehrplan-Sperre):** Themen wie "Statistik" oder "Terme &
#    Gleichungen" werden nun erst freigeschaltet, wenn der aktuell ausgewählte
#    [cite_start]Lernstand ("Klasse") das Mindestniveau erreicht hat (z.B. Statistik ab 7.1)[cite: 775, 779].
# 2. **Toast-Benachrichtigung:** Beim Versuch, ein gesperrtes Thema zu starten,
#    wird eine nicht-blockierende "Toast"-Nachricht am unteren Bildschirmrand
#    [cite_start]angezeigt, die den Benutzer über die benötigte Klasse informiert[cite: 776, 779].
# 3. **Stabile Basis:** Die Datenbank-Migration, Klassen-Speicherung,
#    die erweiterte Aufgabenlogik (basierend auf Klasse und Schwierigkeit)
#    sowie die Hotkey-Steuerung bleiben stabil und funktionsfähig.
#
...
class AufgabenGenerator:
    ...
    def _generate_zahlenraum(self):
        ...
        if op == '/':
            answer = random.randint(lower_bound * 2, upper_bound // 2)
    ...
    def _generate_terme(self):
        ...
        term_str = " ".join([p if p.startswith('-') else ('+' + p) if i > 0 else p for i, p in enumerate(term_parts)]).replace("+ +", "+ ").replace("- -", "+ ").replace("+ -", "- ")
        question = f"Setze x={x_val} (und y={y_val}, falls vorhanden) ein und berechne den Termwert: {term_str}"

        return question, round(solution_value, params['decimals'])
    ...
    def _generate_geometrie(self):
        ...
        if shape == 'Rechteck':
            ...
            radius = random.randint(1, max_dim // 5)
            q_type = random.choice(['Umfang', 'Fläche'])

            if q_type == 'Umfang':
                question = f"Berechne den Umfang eines Kreises mit Radius {radius}{unit}. Runde auf {params['decimals']} Nachkommastellen (Nutze 3.14159)."
                answer = 2 * 3.14159 * radius
            else:
                question = f"Berechne die Fläche eines Kreises mit Radius {radius}{unit}. Runde auf {params['decimals']} Nachkommastellen (Nutze 3.14159)."
