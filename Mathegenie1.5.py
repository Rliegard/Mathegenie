# Gesundheitsüberwachung V1.5 - Vollständige Integration & Datenvisualisierung
#===
#
# Eine plattformunabhängige Desktop-Anwendung (GUI basiert auf PyQt5) zur
# umfassenden Erfassung, Speicherung und grafischen Auswertung von
# Gesundheitsdaten. Diese Version stellt die vollständige und stabile
# Integration aller Kernkomponenten dar.
#
# Haupt-Features der Version 4.0:
# 1. Datenbank-Persistenz (SQLite): Speicherung von Messwerten (Zucker, Cholesterin) und Lebensmitteln/BE.
# 2. Berechnung von Broteinheiten (BE) mit hinterlegtem Standard-KH-Wert.
# 3. Erweiterte Visualisierung: Grafische Darstellung der historischen Messwerte in zwei Matplotlib-Subplots (Zucker und Cholesterin).
# 4. Modernes Design: Umsetzung eines konsistenten Dark-Themes und eines initialen Splash-Screens.
# 5. Navigation: Vollständige Steuerung über QStackedWidget, inklusive Zurück-Buttons und ESC-Key-Handling.
# 6. Usability: Intuitive Split-View-Eingabemaske für Messwerte und Lebensmittel.
#
...
# VERSION: 1.5 (Endversion/Vollintegration)
#############################################
...
class AufgabenGenerator:
    ...
    def _get_params(self):
        ...
        if self.topic == "Terme & Gleichungen" or self.topic == "Geometrie" or self.topic == "Statistik":
            params['range'] = (1, min(100, max_val))
            if year >= 7:
    ...
    def _generate_zahlenraum(self):
        ...
        if op == '/':
            ...
            answer = random.randint(lower_bound * 2, max(lower_bound * 2 + 1, upper_bound // 2))
    ...
    def _generate_terme(self):
        ...
        question = f"Setze x={x_val} (und y={y_val}, falls vorhanden) ein und berechne den Termwert: {term_str}"
        ...
        # Korrigiere "Koeffizient·(Wert)"

        # HIER IST DIE KORREKTUR (Zeile 320): 'eingesetz_str' -> 'eingesetzt_str'
        eingesetzt_str = eingesetzt_str.replace("·(", "*(").replace(" ", "")
    ...
    def _generate_geometrie(self):
        ...
        if shape == 'Rechteck':
            ...
            if q_type == 'Umfang':
                question = f"Berechne den Umfang eines Kreises mit Radius {radius}{unit}. Runde auf {params['decimals']} Nachkommastellen (Nutze Pi={pi_val})."
                ...
                question = f"Berechne die Fläche eines Kreises mit Radius {radius}{unit}. Runde auf {params['decimals']} Nachkommastellen (Nutze Pi={pi_val})."
            ...
            return question, round(answer, params['decimals']), steps # Rückgabe hier, da Rundung für Kreis anders ist
        ...
        if q_type == 'Mittelwert':
            ...
            if n % 2 == 1:
                ...
                steps += (
                    ...
                    f"   - Mittlere Werte (Position {n//2} und {n//2 + 1}): {data[n//2 - 1]} und {data[n//2]}\n"
                    f"   - Median = ({data[n//2 - 1]} + {data[n//2]}) / 2 = {answer}\n"
                )
...
class MatheGenieApp:
    ...
    def show_topic_submenu(self, topic, menu_frame):
        ...
        back_button = ttk.Button(submenu_frame, text="⬅️ Zurück zum Hauptmenü", command=self.show_main_menu)
    ...
    def show_progress_menu(self, event=None):
        ...
        delete_button = ttk.Button(control_frame, text="🗑️ Ergebnis löschen", command=self.delete_selected_result)
        delete_button.pack(side=tk.LEFT, padx=10)
        ToolTip(delete_button, "Löscht das ausgewählte Ergebnis aus der Datenbank.")

        edit_button = ttk.Button(control_frame, text="🔄 Bearbeiten (Simuliert)", command=self.simulate_edit_result)
        edit_button.pack(side=tk.LEFT, padx=10)
        ToolTip(edit_button, "Simuliert eine manuelle Korrektur des Ergebnisses.")

        back_button = ttk.Button(control_frame, text="⬅️ Zurück zum Hauptmenü", command=self.show_main_menu)
    ...
    def show_main_menu(self):
        ...
        schuljahr_button.config(command=lambda: self.schuljahr_dropdown.focus_set()) # Klick auf Button öffnet Dropdown
