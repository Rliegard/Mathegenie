...
# VERSION: 3.0 (Erweiterte Aufgabenlogik und Komplexität)
# ==============================================================================
#
# Eine Tkinter-Anwendung zur Durchführung und Aufzeichnung von Mathematikübungen.
# Diese Version erweitert und verfeinert die Algorithmen zur Aufgaben-Generierung,
# um eine höhere Vielfalt und eine präzisere Anpassung an den Schwierigkeitsgrad
# und die gewählte Klasse zu gewährleisten.
#
# Haupt-Features der Version 3.3:
# 1. **Komplexere Terme:** Der Algorithmus für "Terme & Gleichungen" wurde überarbeitet,
#    um Aufgaben mit Klammern und Punkt-vor-Strich-Rechnung zu generieren,
#    was eine bessere Abdeckung der Lehrplaninhalte ermöglicht.
# 2. **Dynamische Geometrie-Aufgaben:** Zusätzliche Geometrie-Formen (z.B. Parallelogramm, Trapez)
#    oder Volumenberechnungen (Zylinder, Quader) wurden in den Generator aufgenommen.
# 3. **Statistik-Erweiterung:** Der Statistik-Algorithmus kann nun zusätzlich
#    Modus oder Spannweite abfragen, was die Bandbreite der Übungen erhöht.
# 4. **Stabile Basis:** Die Persistenz der Klasse (V3.2) und die modulare Lehrplan-Steuerung (V3.0/3.1)
...
class AufgabenGenerator:
    ...
    def _get_params(self):
        ...
        if self.difficulty == "Leicht":
            ...
            # Respektiert max_val (falls max_val < 99)
            lower_bound = 10
            upper_bound = 99
            if max_val < 10: # Fallback falls max_val sehr klein ist
                lower_bound = 1
            ...
            lower_bound = 100
            if max_val < 100: # Fallback falls max_val < 100
                lower_bound = max(10, max_val // 2) # Starte bei 2-stellig wenn 3-stellig nicht geht
        ...
        if self.topic == "Terme & Gleichungen" or self.topic == "Geometrie" or self.topic == "Statistik":
            ...
            # Diese Regel überschreibt die Schwierigkeits-Range, WENN sie kleiner ist.
            current_lower, current_upper = params['range']

            # Die Obergrenze wird auf 50 (oder die aktuelle Obergrenze, falls kleiner) gesetzt
            new_upper = min(50, current_upper)
            # Die Untergrenze darf nicht größer als die Obergrenze sein
        ...
        # Alte Logik für "Zahlenraum-Training" (Dezimalstellen)
        if self.difficulty == "Mittel" and self.topic == "Zahlenraum-Training":
            params['decimals'] = 0 # Überschreibt die year>=5 Regel

        return params
    # --- ENDE MODIFIKATION _get_params ---
    ...
    def _generate_geometrie(self):
        """Erstellt geometrische Aufgaben.
        ...
        if max_dim <= 0: max_dim = 1 # Sicherstellen, dass randint > 0 ist
    ...
    def _check_answer(self, event=None):
        ...
        if is_correct:
            ...
            FeedbackDialog(self.window,
                           ...
                           drawing_info=drawing_info) # Übergibt Skizzen-Daten
...
class MatheGenieApp:
    ...
    def show_formula_menu(self, event=None):
        ...
        # --- Inhalt für Formeln ---
        # Helper-Funktionen zum Hinzufügen von Inhalt
        ...
        # Helper-Funktion zum Zeichnen (adaptiert von FeedbackDialog._draw_sketch)
        def add_sketch(info):
    ...
    def show_main_menu(self):
        ...
        for i, (text, tooltip_text, handler) in enumerate(button_info):
            ...
            # Layout-Anpassung: 6 Buttons (3x2 Raster)
