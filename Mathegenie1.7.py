...
# VERSION: 1.7 (Robustheit & Fehlerbehebung für Gleitkomma-Genauigkeit)
# ==============================================================================
#
# Eine Tkinter-Anwendung zur Durchführung und Aufzeichnung von Mathematikübungen,
# deren Fokus in dieser Version auf der Verbesserung der Ergebnisgenauigkeit und
# der Fehlerbehandlung liegt.
#
# Haupt-Features der Version 3.1:
# 1. **Rundungs-Logik-Korrektur:** Implementierung von strengeren Rundungsregeln
#    im `AufgabenGenerator` und der `AufgabenSession` zur Vermeidung von
#    Fehlern bei Gleitkommazahlen, insbesondere bei Geometrie- und Statistik-Aufgaben.
# 2. **Toleranz-Anpassung:** Die Prüflogik in `_finish_session` wurde auf eine
#    dynamischere Toleranz (`abs(diff) < tolerance`) umgestellt, um typische
#    Floating-Point-Ungleichheiten zu beheben.
# 3. **Eingabe-Validierung:** Verbesserte Behandlung von non-numerischen oder
#    fehlerhaften Eingaben im Eingabefeld (`answer_entry`) während der Session,
#    um Programmabstürze zu verhindern.
# 4. **Stabile Basis:** Die modulare Lehrplan-Steuerung (V3.0) und alle
...
def format_german(number):
    ...
    # Zuerst auf eine vernünftige Anzahl von Dezimalstellen runden, um float-Artefakte zu vermeiden
    try:
        number = round(number, 5)
    except TypeError: # Falls number keine Zahl ist
        return str(number)

    # Format-String erstellen, der Nullen am Ende entfernt
    s = f"{number:f}".rstrip('0')
    if s.endswith('.'):
        s = s[:-1] # "1453557." -> "1453557"
    ...
    try:
        integer_part_formatted = f"{int(integer_part):,}" # z.B. "1,453,557"
    except ValueError:
        integer_part_formatted = integer_part # Failsafe

    # Separatoren tauschen
    ...
    if decimal_part:
        ...
        # Wenn die Originalzahl ein float war, aber ganzzahlig, hänge ",0" an
...
class AufgabenSession:
    ...
    def _check_answer(self, event=None):
        ...
        # MODIFIZIERT: Eingabeverarbeitung, um deutsche Tausenderpunkte ('.') zu entfernen
        # und Dezimalkomma (',') zu ersetzen.
        user_input = self.answer_entry.get().strip()
        cleaned_input = user_input.replace('.', '') # Entfernt Tausendertrennzeichen (1.453.557 -> 1453557)
        cleaned_input = cleaned_input.replace(',', '.') # Ersetzt Dezimalkomma (1453557,5 -> 1453557.5)

        q_data = self.generator.questions[self.current_question_index]

        # 1. Eingabe verarbeiten
        try:
            user_answer = float(cleaned_input) # Nutzt die bereinigte Eingabe
        ...
        if is_correct:
            ...
            # MODIFIZIERT: Nutzt format_german() für die Anzeige der korrekten Antwort
            correct_answer_formatted = format_german(correct_answer)

            feedback_msg = (
                f"❌ **Leider falsch!**\n\n"
                # Zeige die Original-Eingabe des Users, nicht die bereinigte
